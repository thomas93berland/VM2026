const START = window.VM_RULES?.START_COINS ?? 5000;
const MAX = window.VM_RULES?.MAX_STAKE ?? 500;

let auth, db, user = null, room = 'public';
let unsubs = [];
let state = { me:null, users:[], matches:[], bets:[], sel:[], posts:[], chat:[] };

const $ = id => document.getElementById(id);
const N = n => Number(n || 0).toLocaleString('nb-NO');
const E = s => String(s ?? '').replace(/[&<>"']/g, m => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]));
const P = (w,c) => c > 0 ? Math.round(w / c * 100) : 0;
...
function text(){
  ...
  let d = { ...m, coins:N(m.coins), netProfit:N(m.netProfit), rank:rank ? '#' + rank : '-' };
  document.querySelectorAll('[data-bind]').forEach(e => e.textContent = d[e.dataset.bind] ?? '');
  let a = (m.name || 'T')[0].toUpperCase();
...
function slip(){
  ...
  if($('slipContent')) $('slipContent').hidden = !state.sel.length;
  if($('slipItems')) $('slipItems').innerHTML = state.sel.map(s => `<div class="slip-item"><div><b>${E(s.label)}</b><small>${E(s.title)}</small></div><b>${(+s.odds).toFixed(2)}</b></div>`).join('');
  let stake = Math.min(+$('stakeInput')?.value || 0, MAX);
  let tot = state.sel.reduce((a,s) => a * (+s.odds || 1), 1);
...
async function bet(){
  ...
  if(stake > state.me.coins) return t('Ikke nok VM Coins');
  let tot = state.sel.reduce((a,s) => a * (+s.odds || 1), 1);
  let win = Math.floor(stake * tot);
  let b = db.batch();
...
function listen(){
  ...
  unsubs.push(db.collection('users').onSnapshot(s => { state.users = s.docs.map(d => udef(d.data(), d.id)).sort((a,b) => b.coins - a.coins); render(); }, err));
  unsubs.push(db.collection('matches').onSnapshot(s => { state.matches = s.docs.map(d => ({ id:d.id, ...d.data() })).sort((a,b) => String(a.time || '').localeCompare(String(b.time || ''))); render(); setTimeout(() => window.VM_RESULT_FIX?.refreshSelect?.(), 120); }, err));
...
function bind(){
  ...
  $('clearSlipBtn')?.addEventListener('click', () => { state.sel = []; render(); });
  $('stakeInput')?.addEventListener('input', slip);
  $('placeBetBtn')?.addEventListener('click', bet);
  $('matchForm')?.addEventListener('submit', addMatch);
  // Resultatvelgeren styres kun av js/core/result-admin-fix.js. Ikke bind gammel handler her.
