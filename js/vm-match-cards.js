(()=>{
  ...
  function injectStyles(){
    ...
    css.textContent=`
      ...
      .teams{position:relative;z-index:1;display:grid!important;grid-template-columns:1fr auto 1fr!important;gap:12px!important;align-items:stretch!important;margin:10px 0 14px!important}
      ...
      .vm-match-status{position:relative;z-index:1;display:flex;align-items:center;justify-content:space-between;gap:10px;margin:0 0 12px;padding:9px 11px;border-radius:16px;background:rgba(255,255,255,.035);border:1px solid rgba(255,255,255,.07);color:#d9dfeb;font-size:12px;font-weight:850;text-transform:uppercase;letter-spacing:.055em}
      .vm-match-status b{color:var(--gold)}
      .odds-row{position:relative;z-index:1;gap:9px!important}
      .odd{border-radius:17px!important;border:1px solid rgba(228,184,78,.18)!important;background:linear-gradient(180deg,rgba(255,255,255,.06),rgba(255,255,255,.025))!important;min-height:68px!important;transition:transform .12s ease,border-color .12s ease,background .12s ease!important}
      .odd:not(:disabled):active{transform:scale(.98)}
      .odd small{font-size:11px!important;text-transform:uppercase!important;letter-spacing:.055em!important;color:#cfd5df!important;font-weight:850!important}
      .odd strong{font-size:24px!important;color:var(--gold)!important;letter-spacing:-.04em!important}
      .odd.selected{border-color:rgba(242,201,106,.75)!important;background:linear-gradient(180deg,rgba(228,184,78,.22),rgba(228,184,78,.07))!important;box-shadow:0 0 24px rgba(228,184,78,.14)!important}
      @media(max-width:520px){.teams{grid-template-columns:1fr!important}.teams>span{width:100%;height:30px;border-radius:14px}.teams>strong:last-child{justify-content:flex-start;text-align:left}.vm-team-flag{font-size:26px}.odd{min-height:62px!important}.odd strong{font-size:22px!important}}
    `;
  ...
  function boot(){injectStyles();enhance();setInterval(enhance,900)}
