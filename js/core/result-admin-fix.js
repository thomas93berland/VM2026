...
(()=>{
  ...
  function groupToday(){
    const day=todayKey(),map=new Map();
    bets.forEach(b=>{
  ...
  function listen(){
    if(!ready())return;
    const db=firebase.firestore();
    if(!unsubUsers)unsubUsers=db.collection('users').onSnapshot(s=>{users=new Map(s.docs.map(d=>[d.id,{id:d.id,...d.data()}]));render()},e=>console.warn('Daily users failed',e));
    if(!unsubBets)unsubBets=db.collection('bets').onSnapshot(s=>{bets=s.docs.map(d=>({id:d.id,...d.data()}));render()},e=>console.warn('Daily bets failed',e));
  }
...
(()=>{
  ...
  function renderBox(card,matchId){
    ...
    if(!box){box=document.createElement('section');box.className='trash-talk';box.dataset.matchId=matchId;card.appendChild(box)}
  ...
  async function loadUserDoc(){
    try{const u=firebase.auth().currentUser;if(!u)return null;const s=await firebase.firestore().collection('users').doc(u.uid).get();userDoc=s.exists?{id:s.id,...s.data()}:null;return userDoc}
    catch(e){console.warn('Trash user failed',e);return null}
  }

  function listen(){
    if(!ready()||unsub)return;
    unsub=firebase.firestore().collection('forumPosts').where('kind','==','matchTrash').onSnapshot(s=>{comments=s.docs.map(d=>({id:d.id,...d.data()}));inject()},e=>console.warn('Trash listen failed',e));
  }
  ...
  async function send(matchId,text){
    ...
    await firebase.firestore().collection('forumPosts').add({kind:'matchTrash',matchId,title:'matchTrash:'+matchId,text:clean,author,userId:u.uid,createdAtMs:Date.now(),createdAt:firebase.firestore.FieldValue.serverTimestamp()});
  }
  ...
  function bindSubmit(){
    document.addEventListener('submit',async e=>{
      ...
      try{await send(matchId,input.value);input.value='';toast('Trash talk sendt')}
      catch(err){console.error('Trash send failed',err);toast((err?.code?err.code+': ':'')+(err?.message||'Kunne ikke sende kommentar'))}
    },true);
  }
  ...
  function hideTrashFromForum(){
    document.querySelectorAll('#posts .post').forEach(post=>{const h=post.querySelector('h3')?.textContent||'';if(h.startsWith('matchTrash:'))post.style.display='none'});
  }
